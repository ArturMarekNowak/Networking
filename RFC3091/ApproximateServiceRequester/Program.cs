...
namespace RFC3091
{
    public sealed class Program
    {
        ...
        {
            var approximateServiceEndPoint = new IPEndPoint(IPAddress.Loopback, 22007);
            
            var ipEndpoint = new IPEndPoint(IPAddress.Loopback, 12346);
            var tcpClient = new TcpClient(ipEndpoint);
        
            await tcpClient.ConnectAsync(approximateServiceEndPoint);
